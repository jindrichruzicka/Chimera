 ...
 * registry, parses the raw payload, and invokes `def.reduce()`. Used by the
 * pipeline at Stage 5 and available as a standalone component for consumers
 * (replay, AI) that need raw reduction without the full pipeline ceremony.
 *
...
/**
 * Thrown by `ActionPipeline.process()` at Stage 2 when
 * `ActionDefinition.parsePayload()` throws for the inbound action.
 *
