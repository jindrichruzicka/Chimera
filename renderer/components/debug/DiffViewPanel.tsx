...
//
// The pickers are seeded once from the shared Timeline selection (its tick
...
// a later Timeline click never stomps an in-progress comparison.
//
// Known limitation (mirrors TimelinePanel's staleness note): the tick list
// is fetched once on mount and the page never unmounts panels, so ticks
// recorded after the Inspector opens don't appear in the pickers. Reopening
// the Inspector window (F9 twice) refreshes the list.
...
export interface DiffViewPanelProps {
    ...
    /** Shared Timeline selection; seeds the initial `toTick` only. */
