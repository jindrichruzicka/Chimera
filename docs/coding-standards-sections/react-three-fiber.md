...
- Animation is renderer-local (**Invariant #132**, on Invariants #42/#43 and #56–#58): no animation event may gate an `EngineAction`, and no mixer, clone, playhead, marker or clip-speed value may enter a `GameSnapshot`, a Zustand store, an IPC payload, a save, or a replay. The rule is held by absent parameters — `ClipMarkerHandlers` name no dispatcher — rather than by this bullet. A beat-denominated consequence belongs to the simulation, through the passage's authored `beatWindow` (Invariant #129).
