...
- `chimera/no-game-renderer-internals` — allows game `screens/*.tsx` and React `shell/*.tsx` files to import only the public component-library barrels `@chimera/renderer/components/ui` and `@chimera/renderer/components/chat`, and blocks all other renderer imports from games.
