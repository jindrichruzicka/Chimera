...
| `renderer/`                  | `simulation/content` (types only in production code), the `simulation/foundation` contracts, the `simulation/bridge` preload API contract, `renderer/` internals | `electron/main/`, `ai/engine/` (except IPC types), `apps/*/data`                            |
| `apps/<name>/`               | `simulation/`, `ai/`, own files; renderer surfaces may import what Invariant #96 permits, per file group                                                         | Other `apps/` directories; every other renderer path                                        |
