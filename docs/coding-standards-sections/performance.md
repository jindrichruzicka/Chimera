...
What actually executes where: the bench files live under `apps/*/__tests__/`, so `pnpm -r test` collects them and CI therefore runs every **timing** gate above on every push. The **heap** case is the exception — it is guarded on `globalThis.gc`, which only `--expose-gc` provides and only `npm run test:perf` sets, so it logs nothing and asserts nothing on CI and is exercised locally alone. `npm run test:perf` additionally sets `CHIMERA_PERF_STRICT=1`, which is what makes every gate hard there.
