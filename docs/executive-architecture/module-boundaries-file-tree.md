...
│       ├── manifest.ts              # GameManifest: displayName/window title, realtime + tickRateMs, optional icon + cursor + logoScreen
...
│   ├── game-manifest-contract.ts    # GameManifest (display name, window title, realtime/tickRateMs, icon, cursor, logoScreen) + resolvers (§4.2.1)
│   └── constants.ts
│
├── tools/
