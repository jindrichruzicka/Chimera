...
| 125 | `validate-assets` range-checks every cue sheet            | enforced-by   | `validate-assets` audio-cue-sheet gate (`tools/validate-assets.test.ts`); non-vacuous since #923 — `apps/tactics/asset-manifest.ts` carries two sheets     |
