...
interface GameAPI {
    ...
    /**
     * Stream of ActionRejections for actions dispatched from this renderer
     * that main refused to apply. IPC-level mirror of the §4.3 WebSocket
     * REJECT frame. `sendAction()` is fire-and-forget, so this subscription
     * is the only way the renderer learns a write was discarded (IPC-layer
     * envelope validation today; ActionPipeline Stage-3 rejections once
     * F03–F15 land).
     */
...
/**
 * IPC-bridge analogue of the §4.3 WebSocket `ServerMessage` REJECT frame.
 * `tick === -1` signals the envelope was too malformed to recover a tick.
 */
interface ActionRejection {
    readonly reason: string; // e.g. 'ipc-validation:<channel>' or 'pipeline:<code>'
    readonly tick: number; // -1 when not recoverable
    readonly actionType?: string; // present when the envelope type was parseable
}
...
| -------------------------------------- | ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `game`                                 | `sendAction()` dispatches `EngineAction`s into `ActionPipeline`.       | `parseInvokeRequest(EngineActionSchema, …)` gates the envelope at `ipcMain` (§9.1 / `electron/main/ipc-schemas.ts`). `ActionPipeline.validate()` then runs the action's registered `ActionDefinition.validate()` + `parsePayload` (§4.7). Unknown action types rejected at `registry.resolve()`. Failures at either layer are reported back to the sender via a REJECT push on `chimera:game:action-rejected` (wire-shape mirror of the §4.3 WebSocket REJECT frame); the `send-action` handler never throws out of its `on` callback. | Sanitiser-gated. Renderer input is structurally typed only; every action passes `parsePayload` + `validate` before `reduce`. | Yes — the only authoritative path.                                                            |
