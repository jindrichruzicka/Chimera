...
- A Node **resolution probe** asserts a sample of both packages' public entry points — the renderer's `./components/ui`, `./components/chat`, `./audio`, the `./game` seam, two `./shell/*` routes and the `./styles/*.css` surface, and `@chimera-engine/electron`'s `./main`, `./dev-harness` and `./preload/api` — all resolve from the tarball — a dropped `exports`/`files` entry makes `require.resolve` throw (Invariant #96).
