...
| [core-components/camera-system.md](core-components/camera-system.md)                                           | §4.22                | `CameraMode/CameraPreset`, `GameCanvas` camera props (+ `role="main" \| "overlay"` multi-canvas — the only canvas root a game mounts, Invariant #127), `CameraController`, `useCamera()`, curated renderer configuration (`ShadowQuality`, `ToneMappingMode`, `OutputColorSpace`, `RenderScale` — engine-owned names, never `three` constants) and construction-time `WebGLContextOptions` (frozen at mount, a later change refused by name), render-loop pacing and the frame-rate cap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
