...
| [core-components/customizable-lobby-contract.md](core-components/customizable-lobby-contract.md)           | §4.37, §4.4          | `GameLobbySetup`, `GameSetupConfig`, `GameLobbyScreenProps`, registry-loaded `LobbyScreen` slot, host-only `setMatchSetting`/`setPlayerAttribute`, `snapshot.setup` projection, invariants #99–#101 |
