...
| Call `useAsset()` with a ref that isn't in `asset-manifest.ts` | Works in dev, but the asset won't be packaged into `resources/` in production — and `validate-assets` won't catch it (it only checks _declared_ refs, not `useAsset` call sites). | Register every `AssetRef` in the manifest with `'critical'` or `'deferred'` priority.                                |
