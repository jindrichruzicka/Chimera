...
| [core-components/quick-start-and-shell-flow.md](core-components/quick-start-and-shell-flow.md)                 | §4.41                | The shell FLOW layer: `QuickStartConfig` and its per-seat attributes, `QuickStartCoordinator` as sugar over the public `LobbyManager` verbs (`chimera:lobby:quick-start`), the engine-owned `engine.sessionMode` stamp and the `useLeaveGame` fork it drives, the atomic `chimera:lobby:close-session { autosave }` verb, the `autosaveSlotId` contract and the `saves:slot-update` push behind a reactive Continue, the shell-state discipline a game page reads under, and the one `ConfirmDialogHost`; invariants #96, #99, #101, #108, #137–#140                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
