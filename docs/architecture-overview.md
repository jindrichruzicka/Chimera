...
| [core-components/websocket-message-protocol.md](core-components/websocket-message-protocol.md)                 | §4.3                 | `ClientMessage` union, `ServerMessage` union, CRC32 checksums, snapshot deltas and keyframes, PING/PONG, REVEAL flow                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [core-components/renderer-state-stores.md](core-components/renderer-state-stores.md)                           | §4.4                 | `SnapshotStore`, `MatchStatusStore`, full store catalogue (10 stores), `useSendAction()`, `lobbyConfig`, `useLobbyApi()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [core-components/multiplayer-latency-and-prediction.md](core-components/multiplayer-latency-and-prediction.md) | §6                   | Host authority and the round trip it costs, why there is no client prediction and what adding it would take, PING/PONG round-trip measurement and its unconsumed `onLatencyUpdate` seam, NAT / port-forwarding via `buildNetworkDiagnostics` and the dormant `resolveEndpoint` / `socketFactory` transport seam                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| [core-components/asset-reference-system.md](core-components/asset-reference-system.md)                         | §4.10                | `AssetRef<T>`, `AssetManager`, `useAsset<T>`, `useModelInstance` per-instance clones and the `useModelAnimation` mixer hook via the public `@chimera-engine/renderer/assets` and `components/r3f` barrels, game asset protocol, CI validation script                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [core-components/save-load-persistence.md](core-components/save-load-persistence.md)                           | §4.11                | `SaveFile` (`session` manifest + `matchId`), `JsonSaveSerializer/CompressedSaveSerializer`, `FileSaveRepository` (atomic `.tmp`), `SaveMigrator`, `SessionRestoreCoordinator` menu-load restore                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| [core-components/performance-hud-device-info.md](core-components/performance-hud-device-info.md)               | §4.16, §4.17         | `PerfHud` metrics overlay, `PerfProbe`, `perfStore`, `DeviceInfo`, `SizeClass` breakpoints                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| [core-components/curves-tweening-interaction.md](core-components/curves-tweening-interaction.md)               | §4.21, §4.23         | `EasingFn`, `lerp/easeIn/easeOut`, `useTween`, `useTweenCallback`, `useEntityInterpolation`, `useGameInteraction`, `InteractionBlocker`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| [core-components/camera-system.md](core-components/camera-system.md)                                           | §4.22                | `CameraMode/CameraPreset`, `GameCanvas` camera props (+ `role="main" \| "overlay"` multi-canvas — the only canvas root a game mounts, Invariant #127), the `LightingRig` default light rig, `CameraController`, `useCamera()`, curated renderer configuration (`ShadowQuality`, `ToneMappingMode`, `OutputColorSpace`, `RenderScale` — engine-owned names, never `three` constants) and construction-time `WebGLContextOptions` (frozen at mount, a later change refused by name), render-loop pacing and the frame-rate cap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| [core-components/audio-system.md](core-components/audio-system.md)                                             | §4.25                | `AudioManager`, `PlayOptions`, `AudioBusId` (master/music/sfx/voice), `EventAudioBinding`, 32-voice pool; cue-bounded playback (`Cue`, `from`/`to`/`loopRegion`), fades (`fadeIn`, `fadeOut`/`fadeTo`) and two-track `crossfade` on one shared `t0`; sim-authored `AudioClipMetadata` cue sheets read through `AssetManager.getManifestMetadata`; `useSound`/`useMusicTrack`/`useAudioCues`/`useSpatialAudio` via the public `@chimera-engine/renderer/audio` barrel; spatial audio (`PlayOptions.spatial` distance falloff on each voice's `PannerNode`, one game-supplied listener pose via `setListener`, moving sources via `setVoicePosition`, per-event options resolver); cue OBSERVATION (`observeCues` / `useAudioCues` over one on-demand rAF sampler) and cue-ALIGNED transitions (`crossfadeAtCue` / `fadeOutAtCue`, `secondsUntilCue`) — observe to decide, schedule to execute; variable playback rate (`PlayOptions.rate`, a resample fixed for the life of the voice, plus the `rateFromSemitones` interval helper); invariants #116–#126, #134–#136 |
| [core-components/input-keybindings.md](core-components/input-keybindings.md)                                   | §4.26                | `InputActionId`, `InputAction`, `KeyBinding`, `EngineBindings`, `InputManager`, `useInputAction`/`useInputManager` via the public `@chimera-engine/renderer/input` barrel                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| [core-components/dev-tooling.md](core-components/dev-tooling.md)                                               | §4.32                | `chimera-dev-mp` CLI (`pnpm dev:mp`), game-owned `dev/` fixtures (profiles + scenarios), instance `--dev-*` flags, announce-file handshake, auto host/join/start flow, production guard                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| [core-components/customizable-lobby-contract.md](core-components/customizable-lobby-contract.md)               | §4.37, §4.4          | `GameLobbySetup`, `GameSetupConfig`, `GameLobbyScreenProps`, registry-loaded `LobbyScreen` slot, host-authored `setGameParam` / owner-authored `setPlayerAttribute`, `snapshot.setup` projection, invariants #99–#101                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| [core-components/animation-system.md](core-components/animation-system.md)                                     | §4.40                | `AnimationTrackSheet` authoring vocabulary (notifies, passages, `blendInSeconds`), the pure compile half, the `ClipBackend` seam with its mesh and sprite implementations, `ClipPlayer` (`play`, `transitionTo`, `stop`, `stopAll`, `tick`) and its blended transitions, `useClipPlayer` / `useSpriteClipPlayer` / `AnimatedSprite` / `useAnimationTimeScale` / `useShaderTime` on the `components/r3f` barrel, beat-owned gameplay windows, authoritative time dilation; invariants #21, #96, #128–#132                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [core-components/quick-start-and-shell-flow.md](core-components/quick-start-and-shell-flow.md)                 | §4.41                | The shell FLOW layer: `QuickStartConfig` and its per-seat attributes, `QuickStartCoordinator` as sugar over the public `LobbyManager` verbs (`chimera:lobby:quick-start`), the engine-owned `engine.sessionMode` stamp and the `useLeaveGame` fork it drives, the atomic `chimera:lobby:close-session { autosave }` verb, the `autosaveSlotId` contract and the `saves:slot-update` push behind a reactive Continue, the shell-state discipline a game page reads under, the one `ConfirmDialogHost`, and what a LIVE shell surface may do — the five payload opt-ins, the three lifetimes it holds, and the ordered handover into a match; invariants #21, #52, #65, #96, #99, #101, #108, #127, #137–#140                                                                                                                                                                                                                                                                                                                                                          |
...
| ------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| **Content** (static definition) | `EntityData` JSON — max HP, damage, sprites, model, sfx. Designer-authored, read-only at runtime.       | `apps/<game>/data/entities/entity.json` → loaded into `ContentDatabase` (§4.8)           |
...
```
apps/<game>/data/entities/entity.json           ← static definition
...
#### PlayfieldScreen (game-declared, in `apps/<game>/screens/`)
...
| Store `hp: 47.5` (fractional HP)                               | Violates invariant #44 — floats are not bit-exact across CPUs; causes cross-platform desync.                                                                                                                                         | Scale up: use integer HP `475` with "tenths of HP" semantics — a scaled integer (invariant #75).                                       |
| Load `entity-critical.webp` on first damage tick               | First-hit stutter; 200ms frame spike.                                                                                                                                                                                                | Declare it `'critical'` priority in the game's asset manifest → preloaded before match (§4.10).                                        |
...
| Add a new action type by editing `StateReducer` in engine core | Breaks the Action Registry pattern (§4.7); engine must stay game-agnostic.                                                                                                                                                           | Add an `ActionDefinition` in `apps/<game>/actions/` and register it.                                                                   |
| Read a game setting inside `reduce()` to change outcome        | Violates invariant #36 — settings are UI-only; they are not replayed or synchronised.                                                                                                                                                | Put it in the lobby-agreed `gameParams` map so all clients agree.                                                                      |
| Call `useAsset()` with a ref no asset manifest declares        | Works in dev, but the asset won't be packaged into `resources/` in production. `validate-assets` statically catches this when the ref is a literal, `buildAssetRef(...)`, or a manifest-const member (dynamic refs are only warned). | Declare the `AssetRef` in the manifest the loading surface's session is opened with, at `'critical'` or `'deferred'` priority (§4.10). |
...
my-poker-game           ←  apps/poker/                    (depends on @chimera-engine/simulation, @chimera-engine/cards, @chimera-engine/renderer)
my-ccg                  ←  apps/my-ccg/                   (depends on @chimera-engine/simulation, @chimera-engine/cards, @chimera-engine/renderer)
```
...
**Built / independently consumable** — `@chimera-engine/simulation`, `@chimera-engine/ai`, `@chimera-engine/networking`, `@chimera-engine/renderer`, and `@chimera-engine/electron`. Each declares `"type": "module"`, an `exports` map onto `./dist`, a `"files"` allowlist that ships `dist`, and a `tsconfig.build.json` (`composite`, emitting JS + `.d.ts` + maps). All are wired into the root [`tsconfig.build.json`](../tsconfig.build.json) `tsc -b` reference graph (the inward, acyclic `workspace:*` DAG of Invariant #1) and get an explicit `tsc --noEmit -p <pkg>` in the root `typecheck` script. `@chimera-engine/renderer` publishes the public surface its `exports` map declares; `tsc` emits no CSS, so [`tools/copy-renderer-css.ts`](../tools/copy-renderer-css.ts) copies `*.module.css` + `styles/tokens.css` into `dist/` afterward. `@chimera-engine/electron` (built as of **F62**) ships its main process via `./main` and exposes the preload contract through `./preload/*`; its preload `*-types.ts` are listed in `files` and served from source in `exports` so consumers can type the IPC surface.

The engine is game-agnostic, so the game/consumer layer is **not** part of this package set. It lives under [`apps/`](../apps) — one directory per reference consumer — and each app depends on the `@chimera-engine/*` packages above and registers its game into the electron host and renderer through the public registries. Each is type-checked standalone via its own `apps/<game>/tsconfig.json` (wired into the `typecheck` script for the `.tsx` screen/shell files) and sits at the top of the `tsc -b` solution graph through its own `tsconfig.build.json`.
...
- **Typecheck, lint (type-aware rules), and production** resolve `@chimera-engine/*` to the built **`dist/`** through each package's `exports` map, so `dist` must exist first — hence the `build:packages` prefix on `typecheck`, `lint`, `test`, and `coverage`. The path aliases were removed from the production/typecheck tsconfigs; a consumer app's `e2e/tsconfig.json` keeps a `paths` map of its own, because the Playwright runner resolves bare specifiers with no bundler hook. [`tools/e2e-bootstrap-resolution.test.ts`](../tools/e2e-bootstrap-resolution.test.ts) enumerates the trees that owe one and resolves each mapping.
...
- It runs `build:packages`, then `pnpm pack` per engine package, and synthesizes a throwaway consumer **outside the workspace** whose `package.json` lists each `@chimera-engine/*` as a `file:` tarball dep **and** an npm `overrides` entry — forcing every inter-package edge (the tarballs keep their internal `workspace:*` specs) onto the packed artifacts. `npm install` then resolves every `@chimera-engine/*` **only** through each package's published `exports`/`files`, with no `workspace:*` reach-through.
- A Node **resolution probe** asserts the packed public surface resolves from the tarball — a dropped `exports`/`files` entry makes `require.resolve` throw (Invariant #96). The renderer part of the probe list is complete by construction: before probing, `verify:pack` derives coverage from the `exports` map of the packed renderer manifest itself and fails when any exports key has no probe entry (a wildcard key such as `./shell/*` or `./styles/*.css` is represented by concrete entries under its pattern), so a new renderer barrel cannot ship unprobed. `@chimera-engine/electron` is probed as a curated sample of its public entry points.
...
- Scoping: the renderer GUI shell (`renderer/out`) stays **source-built**, because [`renderer/next.config.ts`](../renderer/next.config.ts) deliberately resolves several renderer barrels + the game registry onto source, so that every module-level singleton they carry — React contexts and Zustand stores alike — exists once in that bundle; the packed renderer surface is therefore gate-checked by the resolution probe rather than re-rendered. The Runtime Debug Layer preload (`./preload/debug-api`) is intentionally **not** a public export (Invariant #27), so the gate excludes the debug specs (`--grep-invert debug`).
