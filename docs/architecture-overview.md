...
| Call `useAsset()` with a ref that isn't in `asset-manifest.ts` | Works in dev, but the asset won't be packaged into `resources/` in production. `validate-assets` statically catches this when the ref is a literal, `buildAssetRef(...)`, or a manifest-const member (dynamic refs are only warned). | Register every `AssetRef` in the manifest with `'critical'` or `'deferred'` priority.                                |
