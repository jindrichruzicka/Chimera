...
On a hop the debt waits on the beat: `SceneRouter` passes `coverUp = !hopBeat.revealed`, and the beat's ONE cover — `LoadingBeatCover`, `surface: 'scene'`, at `--ch-z-loading-hud` (150), above the curtain's `--ch-z-scene-fade` (130) — spans the whole wait, commit included, carrying the last measured fraction across it rather than handing off to a copy. On a within-scene screen switch the debt waits on the held layer, which keeps the copy-and-remainder shape for the one wait that has no curtain owner.

A mounted Suspense fallback is deliberately NOT a term on its own: it renders in the screen slot rather than as a route-level layer — the preset is `position: absolute` with no z-index — so it paints BENEATH the curtain, and holding the curtain up for it alone would show the player black for the whole wait instead of the loading screen the wait exists to explain. Its chunk reaches the debt only through a DECLARED hop's `settled`, where the beat's cover — above the curtain — is what the player looks at for however long the `React.lazy` wait Invariant #133 leaves unbounded actually takes. A hop whose cascade resolves no cover folds nothing in: where there is no layer there is no deferral.
