---
title: 'Replay System'
description: 'ReplayFile schema (formatVersion/seed/actions/metadata), RecordedAction, ReplaySerializer variants, ReplayPlayer (initialize/step/seek/play), ReplayManager (startRecording/recordAction/finaliseRecording/exportCurrentMatch), path-traversal-guarded ReplayAPI IPC, Tactics post-game and main-menu UX integration, the globalThis bridge-contract access pattern, cross-version compatibility via ReplayMigrator, and the coexisting privacy-preserving PerspectiveReplayFile (kind: perspective; projected, fog-safe PlayerSnapshot frames for one locked viewerId).'
...
## Tactics UX Integration

**Post-game summary buttons** — `PostGameReplayActions` in `games/tactics/screens/TacticsPostGameSummary.tsx` renders **Replay** (primary `<Button>`) and **Save Replay** (secondary `<Button>`), mounted only when `snapshot.gameResult !== null`:
...
**Main-menu Replays button** — `tacticsMainMenuDefinition` in `games/tactics/shell/main-menu.ts` contributes a **Replays** button (navigates to `/replays`). Its `disabled` predicate is async and **fail-safe**: it resolves to `true` (disabled) when `replay.perspective.list('tactics')` is empty _or_ the bridge is unavailable. The definition is contributed through the renderer game registry (not a shell-page import) and uses token-mapped layout (invariants #80/#91/#94).
...
| #80 | The engine shell never imports `games/*`; the Tactics main-menu **Replays** button is contributed as data through the registry, not via a shell-page import.                                                                                                                                                                                                                                                                                                                                   |
| #91 | Shell pages reference `var(--ch-*)` tokens only — the Tactics menu definition uses token-mapped layout (`gap: 16` → `--ch-space-md`), no hardcoded colour/spacing/radius.                                                                                                                                                                                                                                                                                                                      |
| #92 | Shell/game UI uses `<Button>` from the renderer UI barrel for actions — the post-game **Replay** / **Save Replay** buttons and the menu **Replays** button.                                                                                                                                                                                                                                                                                                                                    |
| #94 | Engine shell pages do not import `games/*`; game contributions flow through the registry and shared contracts.                                                                                                                                                                                                                                                                                                                                                                                 |
| #96 | Game surfaces import renderer UI only through the public `@chimera/renderer/components/ui` barrel and read IPC bridges off `globalThis` via shared contracts (`shared/replay-bridge-contract.ts`) — never `renderer/*` or `electron/*` internals.                                                                                                                                                                                                                                              |
...
- [Renderer Shell Pages UI Contract](renderer-shell-pages-ui-contract.md) — `<Button>` / token / registry rules behind the Tactics replay buttons (invariants #80/#91/#92/#94)
