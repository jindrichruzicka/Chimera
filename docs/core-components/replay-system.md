...
export interface ReplayFile {
    ...
    readonly gameVersion: string; // from apps/<name>/package.json
...
- `step()` throws `DeterminismError` if `pipeline.process()` does not advance the tick by exactly +1 (invariants #42/#70). That is a REFUSAL, not a guarantee, and it bounds what is replayable today: a recorded action whose reducer `ctx.dispatch`es a child action advances the counter by more than 1, and a recorded `engine:undo` does not advance it by 1 at all. Neither replays — the player throws rather than diverging silently.
...
**Post-game summary button** — a game's `PostGameReplayActions` (e.g. in `apps/<game>/screens/PostGameSummary.tsx`) renders a single **Replay** (primary `<Button>`), mounted only when `snapshot.gameResult !== null`:
...
**Main-menu Replays button** — a game's `gameMainMenuDefinition` (e.g. in `apps/<game>/shell/main-menu.ts`) contributes a **Replays** button (navigates to `/replays`). Its `disabled` predicate is async and **fail-safe**: it resolves to `true` (disabled) when `replay.perspective.list('<game>')` is empty _or_ the bridge is unavailable. The definition is contributed through the renderer game registry (not a shell-page import) and uses token-mapped layout (invariants #80/#91/#94).

**A game can decline recording entirely** — `matchHistory.replay: false` on its manifest (§4.5). The host resolves the capability once and neither recorder arms: `createDeterministicReplayPort` answers `undefined` for the declaration exactly as it does for a packaged build, so the pipeline skips `recordAction`; and `startSessionRecordings` returns before the perspective recorder starts, which also covers the return-to-lobby re-arm, so `hostPerspectiveActive` stays `false` and the per-broadcast `recordSnapshot` is never reached. A joined client of a declining game records nothing either. Nothing extra guards the save or preview paths: with no recording started, `exportCurrent` and the current-match playback sentinel refuse exactly as they already do for a match that was never recorded — the IPC invoke rejects and no file is written. Declining is a host wiring decision and changes no frame's content or visibility (Invariant #98).

**Deterministic replays are debug-only, and never written in a packaged build** — the replay browser always lists **perspective** replays (the player's own point of view). Deterministic replays are a debug artifact: because they reconstruct the full global state from `seed + actions` (every seat's hidden information — e.g. an opponent's whole deck in a CCG), the trusted main process **records and writes them only in a non-packaged build, never in the packaged production app** — the recorder's `replayPort` is `undefined` when `app.isPackaged`, so nothing is ever recorded or persisted there (privacy; Invariants #71/#98, which constrain the file _format_ but do not mandate that the file be written). A post-game save co-saves the deterministic copy alongside the player's perspective whenever one is being recorded (`exportPerspectiveWithDeterministicCoSave`). The renderer's matching browser-surfacing guard is `renderer/app/replays/deterministicReplayGate.ts` — `areDeterministicReplaysVisible()` returns `false` when `NEXT_PUBLIC_CHIMERA_PACKAGED === '1'` (set only by the `package:tactics*` scripts, mirroring the component-gallery gate). When hidden, the page skips the `replay.list()` IPC entirely; and in a packaged build there is nothing on disk to list anyway.
...
| #3  | No replay IPC channel carries a host-internal `GameSnapshot`; `openPlayback` returns `ReplayPlaybackInfo` and the scrub calls return `PlayerSnapshot`s.                                                                                                                                                                                                                                                                                                                                        |
...
| #42 | `ReplayPlayer.step()` throws `DeterminismError` if `pipeline.process()` advances the tick by any amount other than +1. What that excludes from replay — nested `ctx.dispatch` and `engine:undo`/`engine:redo` — is stated under Determinism guarantees above.                                                                                                                                                                                                                                  |
...
| #67 | `ReplayManager` is constructed with an injected `Logger` child and reports through it — no raw `console.*`.                                                                                                                                                                                                                                                                                                                                                                                    |
...
| #80 | The engine shell never imports `apps/*`; a game's main-menu **Replays** button is contributed as data through the registry, not via a shell-page import.                                                                                                                                                                                                                                                                                                                                       |
...
| #94 | Engine shell pages do not import `apps/*`; game contributions flow through the registry and shared contracts.                                                                                                                                                                                                                                                                                                                                                                                  |
| #96 | Game surfaces import renderer UI only through the public `@chimera-engine/renderer/components/ui` barrel and read IPC bridges off `globalThis` via shared contracts (`simulation/foundation/replay-bridge-contract.ts`) — never `renderer/*` or `electron/*` internals.                                                                                                                                                                                                                        |
