...
fade-in, and an opaque cover over the scene — until the assets are there. Where the cascade
resolves a game-declared cover, that hold is the loading beat: black, the cover for at least
`max(settle-or-budget, resolveLoadingBeatFloorMs)`, black, then the scene and its HUD together,
never a widening of any budget here (§4.36, Invariant #133). The gate's own answer, and every
budget below, are untouched by it. Four
