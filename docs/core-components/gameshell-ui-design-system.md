...
A scene hop inside a match answers it too, and passes through the same black: `TransitionOverlay` and `SceneRouter`'s cover both paint `--ch-color-scrim`, the curtain stays painted until the cover has left, and the reveal is owed at the host's commit rather than spent there. What that site does NOT yet share is the sequencer — it latches with `useMinimumVisibleHold` where the routes sequence with `useLoadingBeat`, so its closing black is a single frame rather than a leg, and it raises no cover for a hop whose preload measures nothing. The floor is the same on both. Invariant #133 records the remainder of that divergence rather than asserting a universal over it.
