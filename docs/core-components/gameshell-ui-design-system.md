...
One optional registry-wide knob floors how long a SHOWN cover stays up: once the cascade resolves a game-declared cover form and that cover is actually visible, it stays at least `loadingScreenMinVisibleMs` (tactics declares 400 ms). A wait that outlives the minimum changes nothing, and an absent or `0` value arms no hold — `useMinimumVisibleHold.test.tsx` › holdMs 0 is structurally inert: returns shown and never calls setTimeout. `resolveLoadingCoverHoldMs` (`renderer/components/scene/loadingCoverHold.ts`) resolves it: `0` for absent/zero/negative/non-finite declarations, `0` under `NEXT_PUBLIC_CHIMERA_E2E === '1'` read at call time — the hold is a deliberate delay like the screen fades, never a release budget (Invariant #133) — and deliberately NOT collapsed under `prefers-reduced-motion`, where zeroed fades make a sub-perceptual cover flash strictly worse. `useMinimumVisibleHold` is the shared delayed-release latch beneath every consumer.

Visibility is the arming condition. The hold arms only for a cover the player can see: never for the engine's empty placeholder or a `'none'` resolution, and never while something opaque paints over the cover. On the faded lobby→game entry the app scrim covers the whole route subtree (`AppShell` wraps routes in their own stacking context, so the cover's z-index is local while the scrim is a sibling above), so a cover mounted there starts un-floored; `SceneRouter` receives the same fact as its `sceneCoverOccluded` prop, threaded from the page through `GameShell`, because the shell's inner `FadeProvider` shadows the app-level fade context below it.

What makes that condition reachable on the entry every player uses is the reveal grace. `/game` times its own wait: once the shell is mounted, the gate is still pending, the cascade resolves a game-declared form, a minimum is declared to keep that form up, and the entry scrim is still fully opaque, `ROUTE_COVER_REVEAL_GRACE_MS` (`renderer/assets/criticalAssetPreload.ts`, 350 ms) runs on a fixed timer that reads nothing but the clock. If the wait is still going when it fires, the route eases its OWN scrim off — the same single fade-in it would otherwise have spent at the reveal, brought forward — and the floor stamps from that clear rather than from the cover's mount. A wait that settles first pays nothing: the scrim stays black, the cover is dropped unseen, and the entry is what it was before the grace existed. The knob is what opts an entry in, which is why a declared cover with no declared minimum stays on that unchanged path. `useRouteCoverRevealGrace` is the timer, and `ROUTE_COVER_REVEAL_GRACE_MS` is a plain constant with no env read of its own: an entry arms the grace only where the floor resolved positive, and `resolveLoadingCoverHoldMs` already returns `0` under `NEXT_PUBLIC_CHIMERA_E2E`, so the flag disarms the grace at its arming condition. The unit suites are where the grace is measured; no e2e spec reaches it. `/replays/player` has no entry fade and so no grace: its cover is visible from mount, and both halves belong to whoever gives that route one.

A cover the player saw leaves on a fade rather than a cut. `useCoverExitRamp` keeps it mounted for `screenFadeMs()` past its release, driving it transparent over the scene already rendering beneath — one fade instead of returning through black for two — and `sceneCoverOccluded` follows it through that ramp so the router surfaces no held layer under a cover that is still painting — which also costs a transition cover committed inside those milliseconds its own minimum, the same trade the scrim already makes. A cover nobody saw, and a reveal released for a waiting restore, are dropped on the render that releases them, exactly as before: the ramp arms only on visibility, and a restore's abort control sits under the cover.

The two cover-site families implement the floor differently. The route-entry sites defer the reveal itself: `/game`'s cover drop — plus its latched app-level fade-in, on an entry whose grace has not already spent it — and `/replays/player`'s cover drop, each wait for max(gate settle, shown + minimum) — `restoreWaiting` bypasses the hold, and replay `isReady` is never widened. The `SceneRouter` sites cannot defer their drops — the transition cover unmounts on a host-side commit and a Suspense fallback unmounts the instant its chunk resolves — so the router keeps ONE held-layer slot: the same cascade resolution the dropped cover rendered (with its last measured fraction, or `reason="code"`), snapshotted during covered renders and re-rendered as a sibling layer at the cover z-index until the remainder elapses. One visual wait gets one clock: covers that chain inside a single wait (a commit whose entering screen's chunk suspends) share the first cover's clock, while a superseding transition starts a new wait with its own full minimum, and at most one cover layer is in the DOM at a time. A component-form cover that is itself lazy stamps at the fallback wrapper's mount regardless — the inner placeholder frame it may paint first is accepted.
