...
export interface GameScreenRegistry {
    ...
    readonly loadingScreenMinVisibleMs?: number; // Floor on a raised cover, absent → 400ms (§4.36)
...
// Tier 8 — subscribing to the rebindable actions the game declares (§4.26):
...
| #96  | Game renderer surfaces may import the shared renderer library only through its public barrels, which Invariant #96 enumerates; all other renderer internals stay off-limits, and each barrel is a curated re-export rather than a directory opening.                                                                                                                                                                                                                                    |
...
The same cover has a second site, which is not a Suspense fallback: a route renders `LoadingBeatCover` — the beat's own opaque surface — as a sibling LAYER over the already-mounted `GameShell` while the route-entry asset gate waits (§4.10). It resolves the screen key the ROUTE owns — `snapshot.sceneDefaultScreen ?? registry.sceneDefaultScreens[sceneId] ?? 'playfield'` — and never `useActiveScreen()`, because `uiStore` is a module singleton neither route resets, so a second match in one process would resolve the previous match's screen while its own scene is still loading.

Its third site is the scene TRANSITION cover: `SceneRouter` renders the same `LoadingBeatCover`, `surface: 'scene'`, for the beat's whole cover window — raised against a curtain already black (the beat's `loading-in` leg begins once the fade-out the ack awaits is observed opaque), spanning the host's commit, and leaving by its own closing ramp (§4.18–§4.19). It arms on the cascade resolving a game-DECLARED cover form, exactly as the route entries do, so a hop with nothing to preload still gets its beat — and a registry declaring no cover form raises no layer at all. Two placement rules are load-bearing: it is a SIBLING of the overlay branch, because that branch renders `TransitionOverlay` only when the game supplies no `transitionOverlay` — nested inside it, the cover would be unreachable for exactly the games that do; and it is never `TransitionOverlay`'s child, whose `aria-hidden="true"` would strip a preset's `role="status"` out of the accessibility tree. It resolves the ENTERING scene — `sceneTransition.toSceneId`, and for the screen key `sceneTransition.defaultScreen ?? registry.sceneDefaultScreens[sceneId] ?? 'playfield'` — because `snapshot.sceneId` and `useActiveScreen()` both still describe the scene being left.

### Minimum Visible Time (`loadingScreenMinVisibleMs`)

One optional registry-wide knob floors how long a cover stays up once it is RAISED: `loadingScreenMinVisibleMs` (tactics declares 400 ms). A wait that outlives the floor changes nothing — this is a floor, never a delay added to a slow load.

`resolveLoadingBeatFloorMs` (`renderer/components/scene/loadingCoverHold.ts`) resolves the floor a cover site arms. An absent, negative, non-finite or NaN declaration resolves to `DEFAULT_LOADING_BEAT_FLOOR_MS` (400 ms) — a beat bounded only by its own two fades is sub-perceptual under `prefers-reduced-motion`, where those fades are cuts, so the fallback exists to keep an undeclared cover readable rather than to pick a house rhythm. A declared `0` is the one value that does NOT reach that fallback: it is how a game opts down to gate-settle-only. The resolver collapses to `0` under `NEXT_PUBLIC_CHIMERA_E2E === '1'`, read at call time — the floor is a deliberate delay like the screen fades, never a release budget (Invariant #133) — and deliberately does NOT collapse under `prefers-reduced-motion`, where zeroed fades make a sub-perceptual cover flash strictly worse. `useMinimumVisibleHold` is the shared delayed-release latch beneath the consumers that latch rather than sequence.

**The arming condition is a RESOLVED cover, not a visible one.** The loading beat sequences every `/game` and `/replays/player` entry, and a game-declared form is what adds its cover leg, so a wait that settles before anything could have been shown still gets its cover — the case a visibility-armed hold could not serve, because the only lever it had for making a cover visible was easing the entry scrim off, and the scrim is what hides the scene. The beat holds that scrim black for the whole sequence instead and raises an OPAQUE cover above it, portaled out of the route's stacking context so it can paint over a sibling scrim. What the beat defers is the local reveal and the chrome's mount; `GameShell` mounts on the commit it always did. Those two are separate terms, and the gap between them is load-bearing: `chromeMounted` mounts the layout-bearing HUD row at `covered`, a whole cover leg early, because the row is a grid row and its arrival re-fits the canvas below it — two observer round-trips (the letterbox fit, then r3f's `gl.setSize`) that land tens of milliseconds later, which on a reveal is inside the fade the player is watching. `revealed` still gates the in-game menu host, whose concern is a KEY rather than a paint: an Escape-stack layer under a screen the player cannot see swallows the key without showing a menu. See `useLoadingBeat.ts` and `page.fade-gate.test.tsx`.

A cover the player saw leaves the way it arrived. On `/game` that is the beat's closing leg: the cover ramps down, the curtain is already black behind it, and the scene and HUD are revealed together out of that black. `/replays/player` runs the same sequence. `sceneCoverOccluded` reports to `SceneRouter` what it cannot derive — on `/game` it is simply the beat not having revealed yet.

Both sites answer the same question — when may the player see this screen — and both answer it without moving anything host-visible. On `/game` the reveal lands when the beat's sequence completes, which is bounded below by the gate's own settle and by the floor.

A scene hop inside a match runs the SAME sequencer: `SceneRouter` mounts its own `useLoadingBeat` with `ownsDarkening: false` and `ownsReveal: false` — `useFadeTransition` commands the fade-out because the ack awaits it, and performs the reveal because it is the hook that owes it, while the beat sequences the cover between them. The curtain stays painted until the cover has left, the reveal is owed at the host's commit rather than spent there, and the floor is the same knob on both surfaces. Invariant #133 states the sequence as a universal over every reveal that passes through a curtain.

**The floor is on the RAISE, not on the hop.** When a second hop supersedes a first mid-beat — a different `sceneTransition` key before the first one settled — the beat stays where it is: it never re-enters `covered`, and the floor stamp it took when the cover reached full visibility is not re-armed. The second hop therefore stands on what is LEFT of the first hop's minimum, and its own cover content can stand for less than a whole floor. That is the intended reading of one continuous wait: the cover the player is looking at never left, so there is only ever one raise to floor. Re-arming per hop would instead grow the covered window with the host's churn — every supersession pushing the reveal another floor out — which is the delay-added-to-a-slow-load this knob is not. What follows from it is pinned in `SceneRouter.test.tsx` and `useLoadingBeat.test.tsx`. The standing layer swaps to the superseding hop's resolved form IN PLACE, without unmounting or re-ramping, because what it would uncover is the curtain. A superseding hop whose key resolves no declared form keeps that layer up for the remainder rendering the engine's empty placeholder; it is not dropped, since the curtain behind it is opaque for that whole window anyway, so dropping it buys nothing and costs a layer changing mid-wait. And an UNDECLARED hop raises nothing to inherit: it parks the beat at `covered`, so a declared hop superseding it takes the first raise of the sequence and earns the whole minimum.

A within-scene SCREEN switch (`navigateToScreen`, playfield → tech-tree) keeps the held layer and no fade at all. It is local UI navigation with no curtain owner — nothing is fading, so there is no black to sequence against, and a beat there would have to darken a running game to open a panel. It arms that floor too.
