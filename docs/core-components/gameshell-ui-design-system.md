...
The same cover has a second site, which is not a Suspense fallback: a route renders it as a sibling LAYER over the already-mounted `GameShell` while the route-entry asset gate waits (§4.10). `/game` renders `LoadingBeatCover`, the loading beat's own opaque surface; `/replays/player` still renders `RouteEntryLoadingCover`. Both resolve the screen key the ROUTE owns — `snapshot.sceneDefaultScreen ?? registry.sceneDefaultScreens[sceneId] ?? 'playfield'` — and never `useActiveScreen()`, because `uiStore` is a module singleton neither route resets, so a second match in one process would resolve the previous match's screen while its own scene is still loading.
...
One optional registry-wide knob floors how long a SHOWN cover stays up: once the cascade resolves a game-declared cover form and that cover is actually visible, it stays at least `loadingScreenMinVisibleMs` (tactics declares 400 ms). A wait that outlives the minimum changes nothing. `resolveLoadingCoverHoldMs` (`renderer/components/scene/loadingCoverHold.ts`) resolves what this hold arms for — `useMinimumVisibleHold.test.tsx` › holdMs 0 is structurally inert: returns shown and never calls setTimeout — `0` for absent/zero/negative/non-finite declarations, `0` under `NEXT_PUBLIC_CHIMERA_E2E === '1'` read at call time — the hold is a deliberate delay like the screen fades, never a release budget (Invariant #133) — and deliberately NOT collapsed under `prefers-reduced-motion`, where zeroed fades make a sub-perceptual cover flash strictly worse. `useMinimumVisibleHold` is the shared delayed-release latch beneath every consumer.

**On `/game` the arming condition is a RESOLVED cover, not a visible one.** The loading beat sequences every `/game` entry, and a game-declared form is what adds its cover leg, so a wait that settles before anything could have been shown still gets its cover — the case a visibility-armed hold could not serve, because the only lever it had for making a cover visible was easing the entry scrim off, and the scrim is what hides the scene. The beat holds that scrim black for the whole sequence instead and raises an OPAQUE cover above it, portaled out of the route's stacking context so it can paint over a sibling scrim. What the beat defers is the local reveal and the HUD's mount; `GameShell` mounts on the commit it always did. `/replays/player` has not adopted the beat and keeps the visibility-armed hold above; `/game`'s floor comes from `resolveLoadingBeatFloorMs` in the same module, which differs on what an absent declaration means. See `useLoadingBeat.ts` and `page.fade-gate.test.tsx`.

A cover the player saw leaves the way it arrived. On `/game` that is the beat's closing leg: the cover ramps down, the curtain is already black behind it, and the scene and HUD are revealed together out of that black. `/replays/player` keeps its `screenFadeMs()` exit ramp over the scene beneath. `sceneCoverOccluded` reports to `SceneRouter` what it cannot derive — on `/game` it is simply the beat not having revealed yet.

Both sites answer the same question — when may the player see this screen — and both answer it without moving anything host-visible. On `/game` the reveal lands when the beat's sequence completes, which is bounded below by the gate's own settle and by the declared floor. `SceneRouter`'s held layer keeps its own minimum for the transition and Suspense sites.
