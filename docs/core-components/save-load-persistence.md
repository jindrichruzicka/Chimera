...
The temp file is named per WRITE (`<slotId>.chimera.<pid>.<n>.tmp`), not per slot. One slot has more than one writer and nothing serialises them: `HostSessionPipeline` fires `savePort.autoSave(...)` after every accepted `engine:end_turn` without awaiting it, and an explicit `saves.save()` that names no `slotId` also lands on the autosave slot (`SessionRuntime.captureSaveFile` defaults it to `AUTOSAVE_SLOT_NAME`). Sharing one temp path let the first rename move the file out from under the second, which then failed with `ENOENT` and rejected its caller's save — while a file the OTHER writer produced sat on disk. With a temp path of its own each write is whole before it renames, and the rename picks the winner.

The write counter is module state and restarts at 1 in every process, so the name carries the process id as well: two instances sharing one `userData` — nothing refuses a second one, the app requests no single-instance lock — otherwise gave their first save of a slot the same temp path and reproduced that same `ENOENT`. No two live processes carry one process id, so the prefix separates the two counters wherever they come from one operating system. `FileSaveRepository.multi-process.test.ts` measures it, re-importing the module under a second pid to stand in for the second instance and reading the two temp names off disk while both writes are parked before their renames.

That costs the old scheme's self-healing: `<slot>.chimera.tmp` was reopened and truncated by the next write to the same slot, while a per-write name is never reopened by anything. A process killed between the write and the rename therefore leaves a full-size file that `list()` cannot show (it filters on the `.chimera` suffix) and `delete(slotId)` cannot remove (it unlinks only the slot's own file) — one save file of disk per crash, forever. `reapOrphanTempFiles()` is what gives it back: a sweep of `userData/saves/*/` that unlinks temp artefacts in every shape the writer has produced, called once at app start from the composition root (`electron/main/index.ts`), on the same repository instance the `SaveManager` is built on.

It decides on AGE, not ownership, because it cannot ask whether an artefact belongs to a write in flight — the app requests no single-instance lock, so a second instance sharing the same `userData` may be writing one right now, and taking it would restore the very ENOENT the per-write name removed. `ORPHAN_TEMP_MAX_AGE_MS` is one hour, chosen by what a wrong answer costs in each direction rather than by timing a write: reaping late costs disk the next start gives back, reaping early costs a save. It is a heuristic, not a proof — a writer suspended past the window has its artefact taken, and its rename then fails the way any interrupted save's does. `FileSaveRepository.reap.test.ts` is where the property is measured: a write in flight survives the same pass that takes an aged artefact.
