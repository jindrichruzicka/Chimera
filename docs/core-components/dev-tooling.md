...
answer is not "all ten": four are withheld, two and two — the `no-main-*` pair would guard
...
| `no-animation-derivation-in-reduce` | —             | **applies**  | `simulation/**`, `ai/**` — `error`; OFF on `*.{test,spec}.{ts,tsx}` under both    | Deriving a beat window from `tickRateMs` at reduce time makes the length of every gameplay window a function of the host's pacing knob. It carries no path predicate of its own — the zone IS its scope — which is why its `ai/**` arm, unlike its fromFloat sibling's, stays live wherever the game sits.                 |
...
six curated entries **and** the four exclusions with their reasons, so a rule dropped by
...
`no-animation-derivation-in-reduce` declares no predicate at all, so its zones are the whole
of its scope — the same shape `no-dynamic-games-import` has, arrived at from the opposite
verdict.
