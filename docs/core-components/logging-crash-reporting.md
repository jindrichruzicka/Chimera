...
`main()` does not hand the root logger the fan-out directly, and it does not build the fan-out inline: `createMainLoggerSink({ file, memory, harnessStdout, devStderr, fileMinLevel })` composes it, so which leg carries a threshold is a decision a unit test can reach. The chain is:

```
logger ─► LogRingBufferSink ─► combinedSink ─┬─► createPinoSink   (file, info+)
...
| `createPinoSink` (file)          | always, filtered to `info` and above                          | The durable record: `userData/logs/chimera-YYYY-MM-DD.log`. Buffered (`minLength: 4096`), so an exiting process must `flushSync()` to keep it. Thresholded because it is unbounded — daily rotation, no size cap — so a realtime host logging per beat writes for as long as the match runs. `CHIMERA_LOG_LEVEL` lowers or raises that threshold for a bug report; an unrecognised value resolves to `info` rather than failing the boot, and it is read once, at wire-up. The threshold wraps the fan-out LEG, not `pinoSink` itself, so `startPeriodicFlush` and `refuseToStart` still drain the real SonicBoom buffer. |
