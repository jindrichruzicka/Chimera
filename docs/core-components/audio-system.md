...
- **Curves**: `linear` (default); `exponential` with **both** endpoints clamped off zero (falls back to linear when an endpoint is legitimately 0); `equalPower` as a ≥ 64-waypoint piecewise-linear approximation of the sin/cos quarter-wave — its first waypoint is the re-anchored held value, so it composes with cancel-and-reanchor (never `setValueCurveAtTime`). Every curve is feature-detected and degrades to linear.
...
| `fadeTo{ to: 0, curve: 'exponential' }`                     | Target 0 unreachable exponentially → forced linear-to-0; ceiling becomes 0.                                                |
...
| #120 | Every new stage-1 ramp first cancels prior automation and re-anchors at the held value; exponential ramps clamp **both** endpoints to a `1e-4` epsilon (linear fallback when an endpoint is legitimately 0); `equalPower` is piecewise-linear waypoints (never `setValueCurveAtTime`); all curves are feature-detected and degrade to linear. | code-verified          |
