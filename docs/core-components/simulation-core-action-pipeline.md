...
Terminal gate         — if snapshot.gameResult !== null, reject with match_already_resolved,
                         except engine:sync_request, engine:return_to_lobby, and whatever
                         isSceneTransitionCompletionAction admits (foundation/scene-lifecycle)
...
interface ActionHistory {
     ...
     * constructed with, the oldest entry is evicted and an `action-history:overflow` report
     * carrying `capacity` is emitted — once per saturation episode, not once per eviction.
     * Its level follows `undoable` (Invariant #45).
     */
...
Whether a game runs a wall-clock heartbeat is declared by its **`GameManifest`** (`simulation/foundation/game-manifest-contract.ts` — a pure-data, cross-layer contract; the same manifest sets the OS window title and the renderer's display name). When `manifest.realtime` is `true`, the host wraps a `RealtimeTicker` (lives in `electron/main/` — outside the deterministic core, since a wall-clock timer is host I/O) that dispatches `engine:tick` actions at `manifest.tickRateMs` (converted to Hz via `resolveTickerHz`; default `50 ms` ≈ 20 Hz, the per-tick perf-budget baseline). Each `engine:tick` advances the counter by **1 plus 1 per timer-fired child action**: a timer that fires dispatches through the same `process()`, and Rule 1 above counts every application. The counter is therefore an action count, not a beat count — one outer `engine:tick` is one beat, and reading a beat off a `tick` DIFFERENCE is wrong on exactly the ticks a timer fired. Its per-tick `seed` is taken from the snapshot — never from wall-clock time. The host starts the ticker when the match begins and stops it on session teardown and return-to-lobby; its dispatch is **lean** (`applyAction` + agent re-tick via `SimulationHost.afterTick`) so a heartbeat never triggers turn-based follow-ups (auto-end-turn, commitment reveal). A player action reaching a heartbeat-driven host is applied at the host's **current** tick: the per-action fan-out re-stamps the envelope (`envelopeToApply`, `electron/main/runtime/realtime-input.ts`) before `applyAction`, whether it came from the host's own renderer or a remote client, because the beat advances the clock between the sender's stamp and the envelope's arrival and the pipeline's `StaleActionError` would otherwise refuse every input whose round trip spanned a beat. The fork reads the ticker, not the manifest: a host with no `RealtimeTicker` — `realtime: false`, outside the e2e seam that forces one — applies the envelope as stamped, and a stale stamp there still means the sender acted on a state that no longer exists. The exception is `engine:sync_request`, which every host applies at its current tick, so a client asking for a keyframe after a refused delta is answered rather than refused as stale. Turn-based games (`manifest.realtime: false`, e.g. tactics) run no ticker, and — measured — no production path dispatches `engine:tick` for them either: `SessionRuntime.dispatchTick` is private and reachable only through the e2e hook. So nothing on the host side makes a beat elapse in a turn-based match, and everything beat-denominated — timers, animation windows, the dilation countdown, `onBeat` — waits on a tick the host never sends.
...
| `MAX_ACTION_HISTORY_ENTRIES` | `10_000` | Turn-based default entry cap, and the ceiling for a game's declared `matchHistory.retainActions`; overflow evicts on every append and reports `action-history:overflow` once per saturation episode — `warn` where an undo replays the history, `info` where the resolved policy refuses one |
