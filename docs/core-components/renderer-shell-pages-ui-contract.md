...
| `display.targetFps`              | Target FPS           | Select      | `60`; options `30`, `60`, `120`, `0` (uncapped). Applied by pacing the R3F loop, in two halves both wired by `GameCanvas`: `frameloop={useEngineFrameloop()}` on the `<Canvas>` plus the `FrameRateLimiter` driver inside it (r3f barrel). A game with its own `<Canvas>` wires both — one alone is an uncapped loop or a black canvas (§4.22) |
