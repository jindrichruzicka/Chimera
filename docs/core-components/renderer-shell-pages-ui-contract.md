...
| `display.shadowQuality`          | Shadow Quality       | Select      | `off`; options `off`, `low`, `medium`, `high`. The stored value is an engine-owned NAME, never a `three` constant (Invariant #1). Resolved at the canvas root against the game's `GameCanvas` `shadows` ceiling: the player picks at or below it (§4.22)                                                                                                     |
