...
| `display.targetFps`              | Target FPS           | Select      | `60`; options `30`, `60`, `120`, `0` (uncapped). Applied by the renderer FrameRateLimiter — mounted by `GameCanvas`; a game with its own `<Canvas>` mounts it there itself (r3f barrel) |
...
| #96  | Game renderer surfaces may import the shared renderer library only through its six public barrels — `@chimera-engine/renderer/components/{ui,chat,r3f}` plus the top-level `@chimera-engine/renderer/{game,i18n,audio}`; shell pages continue to receive game customization through renderer registry indirection.                                                                                                                                                                                                  |
