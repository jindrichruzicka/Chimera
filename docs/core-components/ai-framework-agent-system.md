...
- `LobbyAgentSlot` ([`simulation/foundation/lobby-contract.ts`](../../simulation/foundation/lobby-contract.ts)) marks a seat as AI on the synced `LobbyState.agentSlots`. Adding or removing an AI is a **host-only** write (mirroring the host-authored game-param rule, Invariant #99) and rebroadcasts to every peer, so all clients render the same roster — humans in the player list, AI in a separate sub-list. The Add-AI control is disabled when the lobby is full.
...
- **Agent host-local state is discarded and rebuilt on restore.** Everything an agent accumulates outside the `GameSnapshot` — its `AIStateMachine` current state, `CommandScheduler` queue/active command, and any per-entity maps a realtime "director" seat retains inside its `AIState` — is **host-local and out of the deterministic snapshot** (by design). A save/restore rewinds the snapshot but not this host-local state, so the host **tears down and rebuilds the whole agent roster whenever it swaps the authoritative snapshot**: the menu-restore flow seats fresh agents into a freshly hosted lobby (`seatRestoredRoster`), and the in-session same-match live-apply rebuilds them alongside the swap (`rebuildAgentsAgainstRestoredSnapshot`, #907). **Agent authors must therefore never assume host-local state survives a restore** — a rebuilt agent re-runs `createAIState` and re-receives its construction seed from the _restored_ snapshot, so all per-agent state must be reconstructable from that projected seed.
