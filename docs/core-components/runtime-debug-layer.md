...
| ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Main-process graph (`debug-bridge`, `SnapshotInspector`, `SnapshotRingBuffer`, the `chimera:debug*` handlers) | The folded gate prunes both dynamic imports                     | `dist/electron/main.js` loses the debug graph                                  | **Gate** — `pnpm verify:packaged-bundle` (each run also requires a dev build to be rejected by every predicate, so a rotted check fails the gate itself; runs in CI and in the merge script's pre-merge gate), plus the in-memory `packaged-bundle-content.test.ts` |
| Inspector preload                                                                                             | `buildAppBundles` plans no `debug-preload` spec                 | `dist/preload/debug-api.js` (532 KB) + its 1.06 MB sourcemap no longer emitted | **Test** — the same file, plus `electron/build-main/bundle-plan.test.ts`                                                                                                                                                                                            |
...
### The engine-exported plan and guard (scaffolded games)

Both halves of the packaging story — the `define` that folds the gate, and the check that it did — are engine code driven by a thin, app-owned driver. The split is the same in each case: **the engine owns the logic, the app owns the paths**, because a consumer app's copy of either would freeze at scaffold time and never receive an engine fix.

| Subpath                                    | Source                      | Owns                                                                                                              | App-side driver                                                                         |
| ------------------------------------------ | --------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `@chimera-engine/electron/build-main`      | `electron/build-main/`      | the packaging `define`, the esbuild alias/`nodePaths` derivation, the output layout, the bundle plan              | `apps/<game>/electron/build-main.ts` — paths, module resolution, esbuild itself         |
| `@chimera-engine/electron/packaged-bundle` | `electron/packaged-bundle/` | the marker set, the content predicates, the `files:`-allowlist checks, the self-validating `verifyPackagedBundle` | `tools/verify-packaged-bundle.ts`; `templates/blank/electron/verify-packaged-bundle.ts` |

The marker set and the verification logic live in **one place** for the same reason the plan does, with one addition: the debug graph the markers describe is engine code, so the strings that prove its absence are engine internals — a copy in a consumer app would drift silently, and only in the weaker direction (the stale copy stops naming a module and its checks keep passing). `tools/verify-packaged-bundle.test.ts` ratchets the single-definition property repo-wide, and `verify:pack` proves both subpaths resolve from the packed artifact.

The guard export's runner carries its negative controls built in: on **every** run, the dev rebuild that restores the app's `dist/` must be rejected by **every** predicate (per predicate, not merely "some failure"), and a synthetic widened allowlist must be rejected by every allowlist check — so a gutted or rotted check fails the gate itself, on the same run.

Two thin, app-owned gate drivers consume it, each pointing the runner at its app's bundle plan (`appBundleOutfiles`) and real `build:app` invocation:

- `tools/verify-packaged-bundle.ts` — the monorepo's gate (CI + the merge script's pre-merge gate, pinned by `ci-workflow.test.ts` / `merge-gate.test.ts`). It reads the outfile map from the engine plan rather than from `apps/tactics`, which keeps `tools/` off an app import (§3); the reference app takes the engine default, and `tools/verify-packaged-bundle.test.ts` pins that on the driver's source.
- `templates/blank/electron/verify-packaged-bundle.ts` — shipped in every scaffolded game as `pnpm verify:packaged-bundle`, because a scaffolded game's `build-main.ts` driver and `electron-builder.yml` are adopter-editable: dropping the packaging `define` or widening `files:` to `dist/**` would otherwise reship the debug layer with no red check anywhere. It reads its outfile map from `./build-main.js`, so an app that overrides `outfiles` keeps a gate that tracks it. `verify:scaffold` runs the generated app's gate (the `packaged-bundle` step), so a broken template guard fails the engine's own CI (`e2e.yml`, pinned by `e2e-workflow.test.ts`) rather than a downstream adopter's packaging run.

The engine package imports nothing from `tools/` or any app (§3 dependency direction): the drivers own the paths and build commands, the engine owns the plan and the checks. `esbuild` in particular stays app-side — the plan declares its own structural `EsbuildBundleOptions` and never names the specifier, which `electron/__tests__/no-esbuild-dependency.test.ts` ratchets in both the source and the emitted `dist`, because a type-only import would erase before `verify:publish`'s depcheck could see it.
