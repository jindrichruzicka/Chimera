...
**Status: designed, not implemented.** F82 lands as the new core-component section **§4.40** and names invariants **#128–#132** as forthcoming, whose rows the last task authors rather than this section. Animation today is a bare `AnimationMixer`: `renderer/components/r3f/useModelAnimation.ts` allocates one, advances it once per frame at the default render priority and releases it, returning the raw object — so the engine offers no clip selection, no play/stop, no speed and no loop mode. Sprite animation does not exist at all: `LoadedSpriteSheetAsset.frames` is an unparsed `Readonly<Record<string, unknown>>`, and the sprite frame-map type `simulation/foundation/asset-contract.ts` points at is declared in no file. F82 adds the layer a game actually animates against — **clip sheets** (authored notify points and sub-passages), a pure **marker scheduler** turning a stream of playhead samples into `notify` / `passage-start` / `passage-tick` / `passage-end` / `clip-end` emissions, a **three-layer multiplicative speed stack**, and **authoritative time dilation** that re-paces the simulation heartbeat and every clip together.
...
| Author invariants #128–#132, sweep the falsified prose and the roll-call, cut the changeset, open the gate | [#1007](https://github.com/jindrichruzicka/Chimera/issues/1007) |
