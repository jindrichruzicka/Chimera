...
**Out of scope (deferred):** No budget on `loadRendererGame`'s warm-up, which is awaited BEFORE either preload budget starts, so a module that never settles holds the route in a state neither budget can reach — Invariant #133's promise is scoped to the preload for exactly that reason, and whether the wedge is even reachable in a shipped build is unmeasured ([#1109](https://github.com/jindrichruzicka/Chimera/issues/1109)); No host-side barrier re-evaluation for a seat with no mounted `SceneRouter`. `areAllPlayersReady` requires an ack from every key of `state.players`, `engine:scene_ready` has one producer and it runs only inside a mounted router, so a disconnect mid-transition or an AI seat stalls the barrier — reachable on `main` before F83, widened by it only within the budgets, and a change to authoritative barrier semantics is its own feature ([#1110](https://github.com/jindrichruzicka/Chimera/issues/1110)); No settle-all in `preloadCritical`, which awaits its entries in sequence and stops at the first rejection, leaving later critical entries to load on demand. The route gate's own settle-all covers the promoted set and reports it by name, and whether the remaining degradation is reachable past `validate-assets` is unmeasured — filed conditionally rather than as known work ([#1111](https://github.com/jindrichruzicka/Chimera/issues/1111)); No new public barrel subpath and no new exported symbol, so `renderer/__tests__/package-exports-contract.test.ts`, the barrel side-effect module counts and Invariant #96's count word are asserted UNCHANGED rather than moved.
