...
Set up `tools/dev-server.ts` hot-reload harness, the dev multiplayer launcher (with `CHIMERA_DEV_HARNESS` guard; now the `chimera-dev-mp` bin in `electron/dev-tools/dev-harness/`, with per-game seed fixtures under `apps/<game>/dev/`), and the `Logger` interface backed by Pino with daily rotation. Wire `RootErrorBoundary` and `rendererLogger`.
