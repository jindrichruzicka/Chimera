...
description: 'F57–F70: pnpm Workspace Foundation, Extract @chimera-engine/simulation, @chimera-engine/ai, @chimera-engine/networking, @chimera-engine/renderer, @chimera-engine/electron, Tactics Standalone Consumer App + E2E Migration, Package Build/Link/Update Pipeline, create-chimera-game CLI + Blank Template, Engine Package Publishing, App Icon & Per-Game Branding, Save System: In-Game Save & Restorable Sessions, Per-Game Hardware Mouse Cursor, and Game Logo Screen. Executes Appendix C — from a single-package monorepo to a published package hierarchy — and adds a CLI that scaffolds new games from a template.'
tags:
    [
        milestone,
        m9,
...
Close the three user-facing gaps in the existing save backend (F06/F18): saves can only be created from the saves screen's "New Save" form, loading a save cannot start a game (`applyRestoredFile` no-ops without an active session), and multiplayer saves cannot be resumed because provider-assigned `PlayerId`s are minted per-lobby and AI-vs-human seat composition is not persisted. Four deliverables ship together: (1) **UI** — the saves screen is rebuilt on the replay-browser pattern (load/delete rows, confirm dialog, close button; the "New Save" form and per-row overwrite are removed) and a compact host-only Save button joins the tactics HUD footer, prompting for a save name via a modal backed by a reusable engine `SaveGameButton`; (2) **save format v6** — `SaveFile` gains a game-agnostic `session` manifest (`matchId` + seat roster with `host|local|remote|ai` control) and `BaseGameSnapshot` gains a `matchId` minted at `engine:start_game`, so a save fully describes who sat where without profile data (Invariant #59); (3) **restorable sessions** — a `SessionRestoreCoordinator` makes menu-load host a fresh session seeded with the saved roster, apply the checkpoint via the existing Invariant #24 path, and — for saves with remote human seats — hold the game behind a "Waiting for other players" modal (spinner, join code, cancel) until every human reconnects; (4) **seat reattachment** — clients persist per-match session tickets locally and present bounded `claims` in the JOIN frame, which the host resolves against the restored roster (falling back to join-order for unclaimed seats) so returning humans reclaim their original `PlayerId`s and control their original entities. Lobby-agreed game params (e.g. simultaneous turns) and player colors already ride the checkpoint's `setup` and restore for free. Covered by two new Playwright specs: a single-app save→leave→load→verify→delete flow and a two-process multiplayer save→waiting-overlay→reclaim→resume flow.

---

## F69 — Per-Game Hardware Mouse Cursor `§4.37, §4.10, Appendix C.6`

Let a game replace the OS mouse cursor with its own textures, declared in the `GameManifest` — the same manifest that drives the window title, the `realtime` heartbeat flag, and the **F67** icon override. The manifest gains an optional `cursor` declaration mapping named cursor roles (`default`, `pointer`, `disabled`) to game-asset-relative PNG paths with optional per-role hotspot coordinates; when the declaration is absent, the plain system cursor is used exactly as today — the default for every existing and newly scaffolded game. When present, the renderer applies the textures as **hardware cursors** — OS-composited via CSS `cursor: url(<texture>) <x> <y>, <fallback>` — never a canvas-drawn software sprite, so the cursor never lags a frame behind input. Four deliverables ship together: (1) **tokens** — a `--ch-cursor-default` / `--ch-cursor-pointer` token family joins the existing `--ch-cursor-disabled` token; `globals.css` applies the default token at the document root and UI component styles replace hardcoded `cursor: pointer` with `var(--ch-cursor-pointer, pointer)`, so shell chrome and the R3F canvas inherit one consistent cursor set; (2) **plumbing** — the renderer never reads `GameManifest` today, so the game's renderer registration carries the manifest's cursor declaration into the renderer game registry, where a shell-side injector resolves each texture through the game-asset protocol (`chimera://renderer/game-assets/…`, §4.10) into the token overrides and pre-decodes the textures via the existing shell image-warmup seam so the first paint never flashes the system cursor; (3) **tactics adoption** — 32×32 solid-white placeholder PNGs committed at `apps/tactics/assets/cursors/{default,pointer,disabled}.png` (deliberate stand-ins to be overwritten with real art later) and declared in `tacticsManifest`; (4) **template mirror** — the blank template's manifest documents the field with a commented-out example so scaffolded games discover it (Appendix C.6). Covered by a Playwright spec asserting the tactics document cursor computes to a `url(chimera://…)` value and that a game without the declaration keeps the plain system cursor.

---

## F70 — Game Logo Screen `§4.37, Appendix C.6`

Let a game brand its boot with an optional **logo screen** — a fully game-owned page shown before the main menu in packaged builds only. The `GameManifest` — the same manifest that drives the window title, the `realtime` heartbeat flag, the **F67** `icon`, and the **F69** `cursor` — gains an optional `logoScreen` declaration naming the renderer route of a game-provided page. When the declaration is absent, boot behaviour is exactly today's: the window launches straight into `/main-menu` — the default for every existing game. When present **and** the build is packaged (`app.isPackaged`), the Electron main process launches the window into the declared route instead (`chimera://renderer/<route>/?gameId=<id>`); dev and E2E boots are untouched. The engine deliberately does **not** automate the flow — the logo page owns its entire sequence (one logo, several logos in series, or a full intro movie are all just page implementations) and exits by navigating itself to `/main-menu` via `withShellGameId`, so the mechanism stays maximally flexible while the engine ships an out-of-the-box branded default. Four deliverables ship together: (1) **contract & boot routing** — the optional `logoScreen` declaration on `GameManifest` plus host-side launch-URL selection in the composition root's `main()` (extending `buildRendererGameLaunchUrl`); (2) **default screen** — an engine `LogoVideoScreen` component in the `components/ui` barrel that drives the existing app-level `FadeControl` (§4.37): fade in from black, play a full-window stretched `<video>`, then fade out and signal completion on timeout, on any click/keypress (skip), or on video load/playback error — a bad or missing asset can never brick a packaged boot; plus a ready-made engine page at `/logo-screen` (`@chimera-engine/renderer/shell/logo-screen/page`) that mounts it with the Chimera brand video `chimera_logo.mp4` (committed `renderer/public/` copies following the boot-logo pattern; a stub placeholder ships first, to be overwritten with the real cut later) and navigates to the main menu; (3) **adoption** — tactics and the blank template both declare `/logo-screen` and re-export the engine page, so every scaffolded game boots Chimera-branded out of the box (Appendix C.6); (4) **docs** — a new §4.37 subsection documenting the boot-routing rule and the page-owns-the-flow contract. Covered by a Playwright spec driving the logo route directly (the packaged-only gate keeps it out of default E2E boots): the video element is sourced and visible, skip-on-input navigates to the main menu with `gameId` preserved, and the no-declaration boot path stays byte-identical to today.
