...
| `lobby`                     | Create/join/close lobby; kick players (host only). `JOIN` may include `reconnectPlayerId` to reclaim a disconnected seat.                           | `parseInvokeRequest(HostLobbyParamsSchema / JoinLobbyParamsSchema)`. `LobbyManager` verifies caller is host for privileged ops. Local provider reconnect path (`LobbyServer.resolveJoinPlayerId`) allows reclaim only when `reconnectPlayerId` is in `knownPlayers` and not currently connected.  | Main-authoritative; reconnect reclaim is token-gated/local-only risk acceptance (no persistent auth in peer-hosted model). | No (session control only).                                                         |
