...
The root `package.json` is the source for the `test`, `test:e2e` and `test:e2e:action` scripts; read them there rather than from a copy here.
...
| `cloneModelInstance` / `releaseModelInstance` | Distinct skeleton per clone (posing one leaves the other unmoved); geometry/clips shared by reference, materials shared unless a `ModelInstanceMaterialOverride` gives the clone its own copies (tint multiplied, emissive replaced, textures still shared); release disposes the clone-owned materials and skeletons — never a material a caller swapped onto the clone — idempotent and non-throwing; malformed scenes refused with `MalformedModelAssetError`. |
| `useModelInstance` hook                       | Commit-phase allocation: a StrictMode mount/unmount balances clones with releases; a re-render with an unchanged asset never re-clones; an asset-identity change releases the old clone and publishes the new. A material override re-clones only when added or removed; its values change the owned materials in place, keeping the instance identity.                                                                                                           |
...
| `FileSaveRepository`                          | Integration (temp dir): save/list/load/delete; crash-safe write; empty dir returns `[]`; orphan-temp reap — `FileSaveRepository.reap.test.ts`, and `orphan-temp-reap.test.ts` for the sweep it shares.                                                                                                                                                                                                                                                            |
...
| `SnapshotDiff`                                | Identical → empty; added entity → one `added` entry; changed HP → one `changed` entry with before/after. Also over a projected `PlayerSnapshot`, which carries no `seed` and no `timers`.                                                                                                                                                                                                                                                                         |
| `SnapshotDelta`                               | `toSnapshotDelta` carries no `before` and omits `after` on a `removed` entry; `diff → delta → apply` reproduces the new snapshot, including one whose baseline holds an own key valued `undefined`; the applied root is new with untouched subtrees shared by reference; `null` — never a partial write — on each baseline the suite's own `rejects a delta it cannot apply` block names.                                                                         |
