...
| `cloneModelInstance` / `releaseModelInstance` | Distinct skeleton per clone (posing one leaves the other unmoved); geometry/clips shared by reference, materials shared unless a `ModelInstanceMaterialOverride` gives the clone its own copies (tint multiplied, emissive replaced, textures still shared); release disposes the clone-owned materials and skeletons, idempotent and non-throwing; malformed scenes refused with `MalformedModelAssetError`. |
