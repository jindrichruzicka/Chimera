...
| `StateProjector`                     | Property: no `owner-only` / `hidden` field in non-owner `PlayerSnapshot` across 10k random snapshots.                                                  |
| `CommitmentScheme`                   | Valid reveal passes; tampered value throws; tampered nonce throws.                                                                                     |
...
| Crash recovery E2E                   | Playwright: force-kill → relaunch → "Resume" prompt → correct tick restored.                                                                           |
...
| Multiplayer soak                     | 1000 ticks × 4 clients; all clients converge to identical checksum.                                                                                    |
| Obfuscation soak                     | 1000 ticks; no `PlayerSnapshot` for player A contains `owner-only` field from player B.                                                                |
...
| Honest AI isolation                  | `PlayerSnapshot` passed to AI never contains opponent `owner-only` fields.                                                                             |
