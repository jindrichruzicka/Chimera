 ...
 * `./input` exports entry fails it naming the module. It also names two
 * `@chimera-engine/simulation` types through that package's `./*.js` wildcard —
 * `EntityId` and `EngineAction`, which `useGameInteraction`'s signature takes —
 * so the specifier list the shape test pins is four, not three. The specifier
 * list lives in that test rather than being restated here.
...
export const PROBE_SEAM_PLANT = {
    ...
    source: [
        ...
        'export const SeamProbeBlocker = InteractionBlocker;',
        '',
        ...
        '    ];',
        '    // §4.21/§4.22/§4.23, public through the r3f barrel since F91. The',
        '    // curve values matter as much as the EasingFn type: a caller that',
        '    // cannot name easeInOut is stuck on the linear default.',
        '    const easing: EasingFn = easeInOut;',
        '    const tween: TweenState = useTween(200, easing);',
        '    const paced = useTweenCallback(200, easing, {',
        '        onTick: (value: number) => void lerp(0, 1, value),',
        '        onComplete: () => undefined,',
        '        onCancel: () => undefined,',
        '    });',
        '    const camera: CameraController = useCamera();',
        '    const { isBlocked } = useInteractionContext();',
        "    const entityId = 'probe-entity' as EntityId;",
        '    const buildAction = (): EngineAction => ({}) as EngineAction;',
        '    const interaction: InteractionHandlers = useGameInteraction(entityId, buildAction);',
        '    return {',
        '        ready:',
        '            manager !== null &&',
        '            asset === null &&',
        ...
        '            !tween.isRunning &&',
        '            !paced.isRunning &&',
        "            typeof camera.animateTo === 'function' &&",
        '            CameraAnimationCancelled.name.length > 0 &&',
        '            !isBlocked &&',
