 ...
 *   - the published `create-chimera-game` CLI (emits the root with `@chimera/*` resolved from
...
/**
 * Merge the root `dependencies` + `devDependencies` and drop every `@chimera/*` entry. The
 ...
 * only `@chimera/*`) resolves react / vitest / playwright / electron / next by walking up to the
...
export interface StandaloneRootManifest {
    ...
    readonly pnpm: {
         ...
         * `@chimera/* : ^x.y.z` ranges resolve from npm directly.
         */
...
export interface BuildStandaloneRootManifestParams {
     ...
     * the gate to force every `@chimera/*` edge onto a packed `file:<tarball>`.
     */
 ...
 * `@chimera/*` onto the gate's tarballs, and stubs `build:packages` to a no-op — the generated
...
export interface StandaloneAppRewriteParams {
    /** `@chimera/* : ^x.y.z` ranges (the published engine versions) from the toolchain snapshot. */
     ...
     * the app's Electron bundler to alias `@chimera/electron/main` onto, so this env makes
     * build-main resolve the host + preload from the installed `@chimera/electron` instead. Default
 ...
 *
 *   1. rewrite every `@chimera/*` dependency (a `workspace:*` spec inside the monorepo) onto its
 ...
 *      the app's Electron bundler resolves `@chimera/electron`'s host/preload from `node_modules`
 ...
 * surface), never workspace source. `@chimera/*` is INLINED (transformed by vite, not
