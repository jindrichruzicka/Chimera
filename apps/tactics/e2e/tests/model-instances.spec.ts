 ...
 * the tactics board mounts TWO `useModelInstance` components on ONE
 ...
 *
 *   1. **The scene-graph facts pixels cannot show.** The board's showcase
 *      status element carries what only the scene graph knows: both instances
 ...
 * electron-builder packaged app — packaging remains a manual gate item.
 * This spec exercises the `.e2e-build` static-export layout only. No motion
 * is awaited anywhere (the pose is applied before the first report), so the
 * occluded-window frozen-transition hazard does not apply.
 */
...
test.describe('Tactics model seam adoption', () => {
    test('mounts two independent clones of one gltf ref over chimera://', async ({
        ...
        const status = hostWindow.getByTestId('tactics-model-showcase-status');
        ...
        // with UnknownAssetManifestEntryError) instead of a generic timeout.
        await expect(status).toHaveAttribute('data-models-settled', 'true', { timeout: 15_000 });
        await expect(status).toHaveAttribute('data-model-error', '');
        await expect(status).toHaveAttribute('data-models-loaded', '2');

        // Two DISTINCT scene-graph roots, and A's pose stayed out of B.
        await expect(status).toHaveAttribute('data-model-roots-distinct', 'true');
        await expect(status).toHaveAttribute('data-model-pose-a', EXPECTED_POSE_A);
        await expect(status).toHaveAttribute('data-model-pose-b', EXPECTED_POSE_B);

        // And the clones are really on screen: the showcase quads are the only
        // magenta geometry in the scene.
        await game.assertTacticsCanvasHasMagentaPrimitive();
    });
});
