...
// plan: the E2E `global-setup` takes the same door `build:app` does, rather than
// each reaching the engine on its own terms. `tools/packaged-build-flag.test.ts`
// derives the required surface from those consumers' own import lists, so the
// block cannot fall behind what they take. (What lets any of them load before
// `electron/dist` exists is separate, and is the exact-key mapping in
// `apps/tactics/e2e/tsconfig.json` — see `tools/e2e-bootstrap-resolution.test.ts`.)
