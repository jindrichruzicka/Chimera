/**
 * apps/action/e2e/helpers/route-trace.ts
 *
 ...
 * the URL current when the assertion runs, so a route entered and left again
 * within one commit never appears in any sample an out-of-process spec can take.
 * That is exactly the shape of the claim a lobby-less start has to make: not
 * "the window ends up in the match" — which a sampled endpoint proves — but
 * "the lobby was never passed THROUGH on the way".
 *
 ...
 * — every global it needs is reached through structural interfaces (this program
 * carries no DOM lib).
 */
 ...
 * function's source, so a reference to anything else in this module would be
...
function recordRouteTrace(): void {
    ...
    // Consecutive duplicates are dropped: a router that replaces the entry it is
    // already on has not moved the window, and counting it would make a route
    // look revisited. A genuine revisit still records, because the entry before
    ...
    // Back/forward move the window without going through either verb.
 ...
 * two entries differing only by `?gameId=` are one route.
 *
    ...
    for (const url of trace) {
        ...
        const routePath =
            withoutQuery.length > 1 && withoutQuery.endsWith('/')
                ...
                : withoutQuery;
        if (paths[paths.length - 1] === routePath) {
            continue;
        }
