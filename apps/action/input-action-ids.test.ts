...
// The movement actions are named in four places — the settings schema's default
// bindings, the renderer's `InputAction` table, the playfield's per-seat
// subscriptions, and the shell's `/select` page — so they are declared ONCE here
// and each site reads them. These assertions hold the properties none of those
// sites can hold alone.
describe('action movement input actions', () => {
    it('names four distinct actions per seat, all in the game namespace', () => {
        ...
        expect(ACTION_P2_MOVE_ACTION_IDS).toEqual([
            ACTION_P2_MOVE_UP_ACTION,
            ACTION_P2_MOVE_DOWN_ACTION,
            ACTION_P2_MOVE_LEFT_ACTION,
            ACTION_P2_MOVE_RIGHT_ACTION,
    ...
    it('gives the two seats DISJOINT ids, eight in all', () => {
        // A copied constant naming the seat-one id would leave the second seat
        // subscribing to the first's action and both primitives moving together.
        expect(new Set(ACTION_ALL_MOVE_ACTION_IDS).size).toBe(8);
        expect(ACTION_ALL_MOVE_ACTION_IDS).toEqual([
            ...ACTION_MOVE_ACTION_IDS,
            ...ACTION_P2_MOVE_ACTION_IDS,
        ]);
    });
    ...
    it('binds seat one to the arrows and the pass-and-play seat to WASD', () => {
    ...
    it('binds no two actions to the same key — across seats, not only within one', () => {
    ...
    it('maps up/down and left/right to opposite steps, identically for both seats', () => {
        // A copy-paste that gave two actions the same delta would leave one
        // movement key silently doing another's job.
        ...
        expect(ACTION_MOVE_DIRECTIONS[ACTION_P2_MOVE_UP_ACTION]).toEqual({ dx: 0, dy: -1 });
        expect(ACTION_MOVE_DIRECTIONS[ACTION_P2_MOVE_DOWN_ACTION]).toEqual({ dx: 0, dy: 1 });
        expect(ACTION_MOVE_DIRECTIONS[ACTION_P2_MOVE_LEFT_ACTION]).toEqual({ dx: -1, dy: 0 });
        expect(ACTION_MOVE_DIRECTIONS[ACTION_P2_MOVE_RIGHT_ACTION]).toEqual({ dx: 1, dy: 0 });
    });

    it('gives every declared action a direction and a binding, and declares no extras', () => {
        for (const id of ACTION_ALL_MOVE_ACTION_IDS) {
        ...
        expect(Object.keys(ACTION_MOVE_DIRECTIONS).sort()).toEqual(
            [...ACTION_ALL_MOVE_ACTION_IDS].sort(),
        );
