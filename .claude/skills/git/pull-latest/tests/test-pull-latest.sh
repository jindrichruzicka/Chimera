...
# Run with: bash .claude/skills/git/pull-latest/tests/test-pull-latest.sh
#
...
test_clean_pull() {
    ...
    # Run the script
    if bash ../../../.claude/skills/git/pull-latest/scripts/pull-latest.sh 2>&1 | grep -q "Main branch updated successfully"; then
...
test_dirty_working_tree() {
    ...
    if bash ../../../.claude/skills/git/pull-latest/scripts/pull-latest.sh 2>&1 | grep -q "Working tree has uncommitted changes"; then
...
test_return_to_branch() {
    ...
    # Run the script
    OUTPUT=$(bash ../../../.claude/skills/git/pull-latest/scripts/pull-latest.sh 2>&1)
