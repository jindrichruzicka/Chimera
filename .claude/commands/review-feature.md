...
3. Fetch the feature issue with `gh issue view $1 --repo jindrichruzicka/Chimera --json number,title,body,labels,state,milestone,comments,url`; warn if it lacks `feature` label. Discover child tasks via `Part of #<feature>` in issue bodies/comments.
