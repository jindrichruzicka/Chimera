/**
 * electron/main/replay/FileReplayRepository.reap.test.ts
 *
 * Integration tests for `FileReplayRepository.reapOrphanTempFiles()` — the
 * sweep that removes temp artefacts left by a process killed between a
 * replay's write and its rename (§4.28).
 *
 ...
 *
 * Tests written FIRST (red); implementation in `FileReplayRepository.ts`.
 */
...
import { JsonReplaySerializer } from '@chimera-engine/simulation/replay/index.js';
...
import { FileReplayRepository } from './FileReplayRepository.js';
...
/**
 * A `JsonReplaySerializer` with a hook that runs — and can block — inside
 ...
 */
class HookedSerializer implements ReplaySerializer {
    private readonly inner = new JsonReplaySerializer();

    constructor(private readonly onSerialize: () => Promise<void>) {}

    async serialize(file: ReplayFile): Promise<string | Buffer> {
...
const ORPHAN_NAME = '0b5e8f7a-3c1d-4e2f-9a6b-7c8d9e0f1a2b.chimera-replay.tmp';

// ── Tests ────────────────────────────────────────────────────────────────────

describe('FileReplayRepository — reapOrphanTempFiles()', () => {
    let baseDir: string;

    beforeEach(async () => {
        baseDir = await fs.mkdtemp(path.join(os.tmpdir(), 'chimera-replay-reap-test-'));
    });
    ...
    it('reaps an artefact carrying the exact name save() gives its temp file', async () => {
        ...
        let midWrite: string[] = [];
        const repo = new FileReplayRepository(
    ...
    it('leaves an in-flight write untouched while reaping an aged artefact in the same pass', async () => {
        ...
        const pending = repo.save(makeReplayFile('tactics'));
        ...
        await expect(repo.list('tactics')).resolves.toStrictEqual([saved]);
    });

    it('leaves aged replays, foreign temp files and names that only contain the temp shape alone', async () => {
        const repo = new FileReplayRepository(new JsonReplaySerializer(), baseDir);
        const saved = await repo.save(makeReplayFile('tactics'));
        await ageFile(saved, AGED);

        const dir = path.join(baseDir, 'tactics');
        // A `.tmp` that is not this repository's: no `.chimera-replay` before
        // it. Another writer's temp file is not the reaper's to take.
