 ...
 * harmless.
 *
 * Nothing is pushed on start: the first sample arrives one interval in.
 */
export function startHostMetricsPush(
    ...
    timers: HostMetricsPushTimers = {
        setInterval: (handler, ms) => {
            ...
            // Unref'd: a metrics timer must never be the reason the process
            // outlives its last window. Optional because a browser-shaped
            // handle is a number and carries no `unref`.
