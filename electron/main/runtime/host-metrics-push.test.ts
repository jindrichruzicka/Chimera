...
describe('readHostMetrics', () => {
    ...
    it('keeps the fraction the HUD prints', () => {
        // The row renders through `toFixed(1)`, so the tenth is displayed. A
        // whole-megabyte fixture alone could not tell megabytes from rounded
        // megabytes, and the digit a reader watches for growth is the one a
        // coarsened conversion would destroy.
        const metrics = readHostMetrics({
            heapUsedBytes: () => 61.5 * 1024 * 1024 + 3,
            recordedActionCount: () => 0,
        });

        expect(metrics.hostHeapMb).toBeGreaterThan(61.5);
        expect(metrics.hostHeapMb).toBeLessThan(61.51);
    });
...
describe('startHostMetricsPush', () => {
    ...
    it('unrefs its timer, so a metrics push never holds the process open', () => {
        // Exercises the DEFAULT timers, which every injected-timer case here
        // deliberately bypasses — the unref lives there and is invisible to them.
        const unref = vi.fn();
    ...
    it('survives a timer handle that carries no unref', () => {
        const setIntervalSpy = vi
            .spyOn(globalThis, 'setInterval')
            .mockReturnValue(7 as unknown as ReturnType<typeof globalThis.setInterval>);
        try {
            expect(() =>
                startHostMetricsPush(
                    { heapUsedBytes: () => 0, recordedActionCount: () => null },
