...
2. Load [ROADMAP](../../docs/ROADMAP.md), [architecture overview](../../docs/architecture-overview.md), [coding standards](../../docs/coding-standards.md), [architecture invariants](../../docs/executive-architecture/architecture-invariants.md), [invariants skill](../skills/invariants/SKILL.md), and the [code reviewer agent](../agents/chimera-code-reviewer.agent.md).
3. Resolve milestone scope via `gh issue list --repo jindrichruzicka/Chimera --milestone '{{milestone-designator-or-title}}' --state all --limit 200 --json number,title,state,labels,body,milestone,url`; if the designator is ambiguous, cross-check ROADMAP and GitHub milestones.
