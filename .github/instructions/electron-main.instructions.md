...
- Declare IPC handlers in `electron/main/ipc/ipc-handlers.ts`, schemas in `electron/main/ipc/ipc-schemas.ts`, and preload exposure in `electron/preload/api.ts`; keep channel names `chimera:<domain>:<verb>`. Exception: the debug layer (`debug-bridge.ts`) self-contains its `chimera:debug*` handlers and schemas so no debug surface is registered outside debug mode (Invariant #27, §4.12).
