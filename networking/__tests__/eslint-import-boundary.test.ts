 ...
 *
 * ESLint smoke test for the module-import boundary that keeps `@chimera/networking`
 * depending on `@chimera/simulation` only (Invariant #1, issue #768).
 *
 * Runs ESLint programmatically against a fixture that imports a UI-layer
 * workspace package (`@chimera/renderer`) from networking/ and asserts the
...
describe('ESLint import-boundary — @chimera/networking depends on @chimera/simulation only (Invariant #1)', () => {
    it(
        'flags a UI-layer import (@chimera/renderer) from networking/ with no-restricted-imports',
        () => {
